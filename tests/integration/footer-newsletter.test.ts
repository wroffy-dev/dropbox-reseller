...
/*
 * The "resolve a single-input newsletter form" helper these used to cover is
 * gone: the footer renders the chosen form in full, through the same renderer
 * and the same consent path as every other form on the site, so there is no
 * reduced one-field variant left to resolve. What remains below is the claim
 * that still matters — a footer subscription is an ordinary submission and
 * lands as a lead with its attribution intact.
 */
describe('footer newsletter submission', () => {
  it('creates a lead carrying the footer source, page and UTM attribution', async () => {
    ...
    const result = await submitForm({
      ...
      elapsedMs: 5_000,
      values: { email },
      // Every form asks for consent now; a submission that omits it is refused.
  ...
  it('still accepts a genuinely different submission on the same form', async () => {
    ...
    const consent = { enquiry: true, marketing: false, terms: false };
    expect(
      (await submitForm({ formSlug: newsletterSlug, elapsedMs: 5_000, values: { email: first }, consent })).ok,
    ).toBe(true);
    expect(
      (await submitForm({ formSlug: newsletterSlug, elapsedMs: 5_000, values: { email: second }, consent })).ok,
    ).toBe(true);
