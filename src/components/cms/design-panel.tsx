...
export function DesignPanel({
  ...
  const anchorRewritten = Boolean(anchorDraft.trim()) && anchorNormalised !== anchorDraft.trim().toLowerCase();

  return (
    <div className="space-y-3">
      {/* Breakpoint switch — scopes the spacing/layout group below. */}
          ...
          <>
            <Field label="Layout" htmlFor={`${idPrefix}-widthMode`}>
              <Select
                ...
                onChange={(e) => patch({ widthMode: e.target.value as SectionDesign['widthMode'] })}
              >
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Text alignment" htmlFor={`${idPrefix}-${breakpoint}-align`}>
            <Select
              ...
              value={current.align}
              onChange={(e) =>
                patchBreakpoint(breakpoint, { align: e.target.value as BreakpointDesign['align'] })
              }
            >
      ...
      <DesignGroup title="Background">
        ...
        >
          <Select
            ...
            onChange={(e) =>
              patch({
                background: { ...design.background, type: e.target.value as SectionDesign['background']['type'] },
              })
            }
          >
          ...
          <>
            <div className="grid gap-4 sm:grid-cols-2">
              <ColorInput
                ...
                onChange={(gradientFrom) => patch({ background: { ...design.background, gradientFrom } })}
              />
              <ColorInput
                label="Gradient end"
                id={`${idPrefix}-gradTo`}
                value={design.background.gradientTo}
                onChange={(gradientTo) => patch({ background: { ...design.background, gradientTo } })}
              />
            </div>
            <Field
            ...
            >
              <input
                ...
                onChange={(e) =>
                  patch({ background: { ...design.background, gradientAngle: Number(e.target.value) } })
                }
                ...
                <Select
                  ...
                  onChange={(e) =>
                    patch({
                      background: {
                        ...design.background,
                        imagePosition: e.target.value as SectionDesign['background']['imagePosition'],
                      },
                    })
                  }
                >
                ...
                <Select
                  ...
                  onChange={(e) =>
                    patch({
                      background: {
                        ...design.background,
                        imageRepeat: e.target.value as SectionDesign['background']['imageRepeat'],
                      },
                    })
                  }
                >
                ...
                <Select
                  ...
                  onChange={(e) =>
                    patch({
                      background: {
                        ...design.background,
                        imageAttachment: e.target.value as SectionDesign['background']['imageAttachment'],
                      },
                    })
                  }
                >
        ...
        {design.background.type === 'image' || design.background.type === 'gradient' ? (
          <>
            <ColorInput
              ...
              onChange={(overlayColor) => patch({ background: { ...design.background, overlayColor } })}
            />
            <Field
            ...
            >
              <input
                ...
                onChange={(e) =>
                  patch({ background: { ...design.background, overlayOpacity: Number(e.target.value) } })
                }
                className="w-full accent-brand"
              />
            </Field>
      ...
      <DesignGroup title="Visibility & anchor">
        ...
        {anchorNormalised && !anchorDuplicate ? (
          <p className="text-xs text-muted">
            Renders as <code className="font-mono">id=&quot;{anchorNormalised}&quot;</code>, linkable as{' '}
            <code className="font-mono">#{anchorNormalised}</code>.
          </p>
        ) : null}
        {anchorRewritten && !anchorDuplicate ? (
          <p className="text-xs text-amber-600">
            Adjusted to <code className="font-mono">{anchorNormalised}</code> so it is valid in a URL.
          </p>
        ) : null}
